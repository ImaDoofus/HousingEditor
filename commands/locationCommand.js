...
}).setTabCompletions(['round']).setName("location").setAliases(['loc'])
...
function getLocation(arg){
	...
	new Message(new TextComponent(`&f&lPOSITION: &a${x}&f, &a${y}&f, &a${z}&f. &7(Hover)`).setClick("run_command", `/ct copy ${x} ${y} ${z}`).setHoverValue(`&6&lCLICK TO COPY\n${x} ${y} ${z}`)).chat();
	new Message(new TextComponent(`&f&lROTATION: &a${yaw}&f, &a${pitch}&f. &7(Hover)`).setClick("run_command", `/ct copy ${yaw} ${pitch}`).setHoverValue(`&6&lCLICK TO COPY\n${yaw} ${pitch}`)).chat();
}