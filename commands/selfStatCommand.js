...
const selfStatCommand = register("command", (stat, operation, value) => {
  ...
  if (stat && options.includes(operation) && !isNaN(value))
    ChatLib.command(
      `editstats ${Player.getName()} ${stat} ${operation} ${value}`
    );
  else if (stat)
    ChatLib.chat("&cInvalid Usage: /selfstat <stat> <set/inc/dec> <value>");
