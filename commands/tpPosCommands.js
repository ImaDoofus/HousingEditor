...
function onHousing() {
  return ChatLib.removeFormatting(Scoreboard.getTitle()).includes("HOUSING") && Server.getIP().includes("hypixel.net");
}
...
register("messageSent", (message, event) => {
  if (message.toLowerCase().startsWith("/tp") && message.toLowerCase() !== "/tp" && onHousing() && !message.toLowerCase().includes("pos")) {
    cancel(event);
...
register("command", (...parameters) => {
  ...
  if (parameters[0] === undefined || !correctTildaFormat(parameters) || parameters.length <= 2) {
    ...
    if (parameters.length === 3) {
      ...
      for (i = 0; i < parameters.length; i++) {
        if (parameters[i].startsWith("~")) {
          if (parameters[i] !== "~") {
            parameterInfo[coordTypes[i]] = parseFloat(parameters[i].replace("~", "")) + coordMethods[i];
          } else {
      ...
      if (!isNaN(parameterInfo["x"]) && !isNaN(parameterInfo["y"]) && !isNaN(parameterInfo["z"])) {
        ChatLib.say(`/tp ${parameterInfo["x"]} ${parameterInfo["y"]} ${parameterInfo["z"]} `);
      } else {
        ChatLib.say(`/tp ${slashTPParameters}`);
      }
    } else {
      try {
        ...
        if (!isNaN(parameterInfo["x"]) && !isNaN(parameterInfo["y"]) && !isNaN(parameterInfo["z"])) {
          ChatLib.say(`/tp ${PlayerMP.getName()} ${parameterInfo["x"]} ${parameterInfo["y"]} ${parameterInfo["z"]} `);
        } else {
          ChatLib.say(`/tp ${slashTPParameters}`);
        }
      } catch (error) {
